...
const CreatePost = () => {
  ...
  const [form, setForm] = useState({ name: "", prompt: "", pic: "" });
  ...
  const generateImage = async () => {
    try {
      ...
      setForm({ ...form, pic: `data:image/jpeg;base64,${data.pic}` });
    } catch (error) {
      alert(error);
    } finally {
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-5">
          ...
          <FormField
            ...
            type="text"
            name="prompt"
            placeholder="A serene forest scene with glowing mushrooms and fairies."
            value={form.prompt}
            handleChange={handleChange}
            isSurpriseMe
          ...
          <div className="relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center">
            {form.pic ? (
              ...
              <img
                src={preview}
                alt="preview"
                className="w-9/12 h-9/12 object-contained opacity-40"
              />
            )}
            {generatingImg && (
        ...
        <div className="mt-10">
          ...
          <button
            type="submit"
            className="mt-3 textt-white bg-[#6469ff] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center"
          >
